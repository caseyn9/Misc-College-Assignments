...
getMeasured rqdDist [] = []
getMeasured rqdDist [x] = [x]
getMeasured rqdDist (x:xs) 	
							| distance x >= rqdDist 					= [x] 
							| otherwise 								= x : (getMeasured (rqdDist - pathDistance [x]) xs)
getMeasure _ _ = []
...
findFastest rqdDist [x] = Just ([x],pathSpeed [x])
findFastest rqdDist (x:xs)	= 	if pathDistance (x:xs) < rqdDist 
								then Nothing
								else Just (helper rqdDist (x:xs), pathSpeed (helper rqdDist (x:xs)))

helper :: Distance -> SegPath -> SegPath
helper rqdDist [] = []
helper rqdDist [x] = [x]
helper rqdDist (x:xs) = if pathSpeed (getMeasured rqdDist(x:xs)) >= pathSpeed (helper rqdDist xs)
						then getMeasured rqdDist (x:xs)
						else if (pathDistance (helper rqdDist xs)) < rqdDist 
							then x: (helper rqdDist xs)
							else helper rqdDist xs
helper _ _= []
